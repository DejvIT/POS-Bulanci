...
namespace structures 
{
	...
	Array<T>& Array<T>::operator=(const Array<T>& other)
	{
		...
		{
			throw std::logic_error("Zla velkost vektora!");
		}
		if (this != &other)
		{
			*vector_ = *other.vector_;
			//Vector::copy(*(other.vector_), 0, *vector, 0, size_);;
		}
		return *this;
	}
	...
	T& Array<T>::operator[](const int index)
	{
		...
		T *castBytePtr = reinterpret_cast<T*>(bytePtr);
		return *castBytePtr;
	}
	...
	{
		int vectorIndex = mapFunction(index);							//indexVoVektore
		byte *bytePtr = vector_->getBytePointer(vectorIndex);			//ptrVoVectore
		T *castBytePtr = reinterpret_cast<T*>(bytePtr);					//typovyPtr
		return *castBytePtr;
		//return *(reinterpret_cast<T*>(vector->getBytePointer(mapFunction(index))));
	}

	template<typename T>
	inline bool Array<T>::operator==(const Array<T>& other) const
	{
		return (size_ == other.size_) && (*vector_ == *other.vector_);
		//return (size_ == other.size_) && (vector_ == other.vector_); -> toto by porovnavalo smerniky, nie obsahy
	}

	template<typename T>
	void Array<T>::copy(const Array<T>& src, const int srcStartIndex, Array<T>& dest, const int destStartIndex, const int length)
	{
		//int startSrc = mapFunction(srcStartIndex);
		//int startDest = mapFunction(destStartIndex);
		////Vector::copy()
		//Vector::copy(src, mapFunction(srcStartIndex), dest, mapFunction(destStartIndex), mapFunction(length));
		//Vector::copy(*src.vector_, *src.mapFunction(srcStartIndex), *dest.vector_, *dest.mapFunction(destStartIndex), *mapFunction(length));
