...
namespace structures
{
	...
	private:
		...
		/// <summary> Zmensi kapacitu zoznamu. </summary>
		void shrink();
	...
	inline ArrayList<T>& ArrayList<T>::operator=(const ArrayList<T>& other)
	{
		if (this != &other)
		{
			//Array<T>* kopia = new Array<T>(*other.array_);
			delete array_;
			array_ = new Array<T>(*other.array_);
			size_ = other.size_;
		}
	...
	inline void ArrayList<T>::add(const T & data)
	{
		...
		(*array_)[size_++] = data;				//neprida to az moc daleko ?? 
	}
	...
	//DU automaticke shrinkovanie... ak ma 8 prvkov 
	...
	inline T ArrayList<T>::removeAt(const int index)
	{
		...
		size_--;
		if (4 * (size_) <= array_->size() && size_ != 0)
		{
			shrink();
		}
		return vrat;
	...
	{
		Array<T> *novePole = new Array<T>(size_ * 2);
		Array<T>::copy(*array_, 0, *novePole, 0, size_);
		delete array_;
		array_ = novePole;
	}
	...
	{
		const ArrayListIterator& iter = dynamic_cast<const ArrayListIterator&>(other);
		position_ = iter.position_;
		arrayList_ = iter.arrayList_;
		return *this;
	}
	...
	{
		const ArrayListIterator& iter = dynamic_cast<const ArrayListIterator&>(other);
		return position_ != iter.position_ || arrayList_ != iter.arrayList_;
	}

	//jednotkova zlozitost
	template<typename T>
