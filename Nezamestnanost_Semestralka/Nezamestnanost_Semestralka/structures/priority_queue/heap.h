...
namespace structures
{
	...
	void Heap<T>::push(const int priority, const T& data)
	{
		...
		while (current > -1 && current < static_cast<int>(list_->size()) && parentIndex >= 0 && (*list_)[parentIndex]->getPriority() > priority)
		{
	...
	inline int Heap<T>::getGreaterSonIndex(const int index)
	{
		...
		if ((tempIndex - 1) < static_cast<int>(list_->size()) && tempIndex < static_cast<int>(list_->size()))
		{
			return (*list_)[tempIndex - 1]->getPriority() <= (*list_)[tempIndex]->getPriority() ? (tempIndex - 1) : tempIndex;
		}
		else if ((tempIndex - 1) < static_cast<int>(list_->size()))
		{
			return tempIndex - 1;
		}
		else
