...
namespace structures
{
	...
	inline ArrayList<T>& ArrayList<T>::operator=(const ArrayList<T>& other)
	{
		...
		if (this != &other) {
			size_ = other.size_;
			delete array_;	//ak by sme nezmazali pÙvodne pole, ostal by vysieù v pam‰ti ako memory leak
			array_ = new Array<T>(*other.array_);
		}
		return *this;
	}
	...
	inline void ArrayList<T>::add(const T & data)
	{
		...
		(*array_)[static_cast<int>(size_++)] = data;	//metÛda add
	}
	...
	inline T ArrayList<T>::removeAt(const int index)
	{
		...
		size_--;	//odoberie index_ o 1, ak sa index+1==size_ => jedn· sa o posledn˝ prvok v poli, a teda staËÌ odobraù index, pretoûe pri prid·vani Ôalöieho prvku sa to prepÌöe

		//array shrinking
		if (size_ <= (array_->size() / 4)) {						//ak aspoÚ 75% array_listu je nevyuûitÈ
			Array<T> *newArray = new Array<T>(array_->size() / 2);	//vytvor array o poloviËnej veækosti z pÙvodneho
			Array<T>::copy(*array_, 0, *newArray, 0, size_);		//prekopiruj star˝ do novÈho
			delete array_;											//vymaû star˝
			array_ = newArray;										//priraÔ nov˝
		}
		return pomocnik;
	}
	...
	inline int ArrayList<T>::getIndexOf(const T & data)
	{
		...
		return -1;	//konötanta -1 znamen·, ûe sa nenaöiel prvok v liste
	}
	...
	inline void ArrayList<T>::enlarge()
	{
		...
		Array<T>::copy(*array_, 0, *newArray, 0, static_cast<int>(size_));	//namiesto size_ mÙûe byù aj array_->size_
		delete array_;
		array_ = newArray;
	}
